    ...
    <main className="min-h-full bg-[#07101f] px-3 py-4 text-slate-100 sm:px-5 sm:py-6">
      <div className="mx-auto w-full max-w-lg space-y-3">
        ...
        <div className="space-y-3">{nest.itens.map((item) => {
          ...
          return <section key={item.id} className="rounded-2xl border border-slate-700 bg-[#111b2b] p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-base font-bold text-white">{item.peca.codigo}</p><p className="mt-1 text-xs text-slate-400">{item.peca.nome}</p><p className="mt-1 font-mono text-[10px] uppercase tracking-wide text-cyan-300">OP {item.op.numeroSequencia} · lote {item.op.lote ?? "-"}</p></div><div className="text-right"><p className="font-mono text-[10px] uppercase text-slate-500">Restante</p><strong className="text-2xl text-cyan-100">{restante}</strong></div></div>{!encerrado && nest.status === "EM_CORTE" && restante > 0 && <PlasmaProductionForm nestItemId={item.id} restante={restante} />}{!encerrado && nest.status !== "EM_CORTE" && <p className="mt-3 border-t border-slate-700/70 pt-3 text-xs text-slate-400">Inicie ou retome o corte para registrar.</p>}{item.lancamentos.length > 0 && <p className="mt-3 text-xs text-slate-500">Último registro: {item.lancamentos[0].quantidadeBoa} boas · {item.lancamentos[0].quantidadeRefugo} perdas · {item.lancamentos[0].funcionario.nome}</p>}</section>;
        })}</div>
      </div>
    </main>
  );
