    ...
    <main className="min-h-full bg-[#07101f] px-3 py-4 text-slate-100 sm:px-5 sm:py-6">
      <div className="mx-auto w-full max-w-lg space-y-3">
        <div className="flex items-center justify-between gap-3">
          <Link href={`/apontamentos?setor=${nest.setor.id}`} className="text-xs font-semibold text-slate-400 hover:text-cyan-200">← NESTs para operar</Link>
        </div>
        ...
        {!encerrado && podeOperar && <section className="rounded-2xl border border-slate-700 bg-[#111b2b] p-4">
          ...
          <PlasmaEventForm
            ...
            rotaDepoisFinalizar={`/apontamentos?setor=${nest.setor.id}`}
          />
        </section>}
