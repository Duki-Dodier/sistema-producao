-- CreateTable
CREATE TABLE "Setor" (
    "id" SERIAL NOT NULL,
    "nome" TEXT NOT NULL,
    "ordemPadrao" INTEGER NOT NULL DEFAULT 0,
    "metaMensal" INTEGER,
    "lider" TEXT,
    "diasUteisMes" INTEGER,
    "createdAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "updatedAt" TIMESTAMP(3) NOT NULL,

    CONSTRAINT "Setor_pkey" PRIMARY KEY ("id")
);

-- CreateTable
CREATE TABLE "Funcionario" (
    "id" SERIAL NOT NULL,
    "nome" TEXT NOT NULL,
    "setorId" INTEGER NOT NULL,
    "ativo" BOOLEAN NOT NULL DEFAULT true,
    "pin" TEXT,
    "papel" TEXT NOT NULL DEFAULT 'OPERADOR',
    "createdAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "updatedAt" TIMESTAMP(3) NOT NULL,

    CONSTRAINT "Funcionario_pkey" PRIMARY KEY ("id")
);

-- CreateTable
CREATE TABLE "AjusteApontamento" (
    "id" SERIAL NOT NULL,
    "apontamentoId" INTEGER NOT NULL,
    "valorAnterior" INTEGER NOT NULL,
    "valorNovo" INTEGER NOT NULL,
    "motivo" TEXT NOT NULL,
    "autorizadoPorId" INTEGER NOT NULL,
    "dataHora" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,

    CONSTRAINT "AjusteApontamento_pkey" PRIMARY KEY ("id")
);

-- CreateTable
CREATE TABLE "Peca" (
    "id" SERIAL NOT NULL,
    "codigo" TEXT NOT NULL,
    "nome" TEXT NOT NULL,
    "medida" TEXT,
    "imagemUrl" TEXT,
    "setorId" INTEGER NOT NULL,
    "tipoMaterial" TEXT,
    "medidaA" DOUBLE PRECISION,
    "medidaB" DOUBLE PRECISION,
    "espessuraMm" DOUBLE PRECISION,
    "comprimentoMm" DOUBLE PRECISION,
    "processos" TEXT,
    "observacao" TEXT,
    "createdAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "updatedAt" TIMESTAMP(3) NOT NULL,

    CONSTRAINT "Peca_pkey" PRIMARY KEY ("id")
);

-- CreateTable
CREATE TABLE "PecaCurva" (
    "id" SERIAL NOT NULL,
    "pecaId" INTEGER NOT NULL,
    "ordem" INTEGER NOT NULL,
    "medidaCm" DOUBLE PRECISION,
    "anguloGraus" DOUBLE PRECISION,
    "quantidade" INTEGER NOT NULL DEFAULT 1,
    "maquina" TEXT,

    CONSTRAINT "PecaCurva_pkey" PRIMARY KEY ("id")
);

-- CreateTable
CREATE TABLE "RecebimentoAgrupamento" (
    "id" SERIAL NOT NULL,
    "opId" INTEGER NOT NULL,
    "setorOrigemId" INTEGER NOT NULL,
    "categoria" TEXT NOT NULL,
    "recebidoPor" TEXT NOT NULL,
    "localizacao" TEXT NOT NULL,
    "dataHora" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,

    CONSTRAINT "RecebimentoAgrupamento_pkey" PRIMARY KEY ("id")
);

-- CreateTable
CREATE TABLE "ModeloPeca" (
    "id" SERIAL NOT NULL,
    "modeloId" INTEGER NOT NULL,
    "pecaId" INTEGER NOT NULL,
    "quantidadeNecessaria" INTEGER NOT NULL DEFAULT 1,

    CONSTRAINT "ModeloPeca_pkey" PRIMARY KEY ("id")
);

-- CreateTable
CREATE TABLE "PecaRoteiro" (
    "id" SERIAL NOT NULL,
    "pecaId" INTEGER NOT NULL,
    "setorId" INTEGER NOT NULL,
    "processo" TEXT NOT NULL,
    "ordem" INTEGER NOT NULL,

    CONSTRAINT "PecaRoteiro_pkey" PRIMARY KEY ("id")
);

-- CreateTable
CREATE TABLE "Modelo" (
    "id" SERIAL NOT NULL,
    "codigo" TEXT NOT NULL,
    "nome" TEXT,
    "curva" TEXT NOT NULL,
    "tipo" TEXT NOT NULL,
    "tamanhoPonteira" TEXT,
    "imagemUrl" TEXT,
    "estoqueMinimo" INTEGER,
    "createdAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "updatedAt" TIMESTAMP(3) NOT NULL,

    CONSTRAINT "Modelo_pkey" PRIMARY KEY ("id")
);

-- CreateTable
CREATE TABLE "ModeloRoteiro" (
    "id" SERIAL NOT NULL,
    "modeloId" INTEGER NOT NULL,
    "setorId" INTEGER NOT NULL,
    "ordem" INTEGER NOT NULL,

    CONSTRAINT "ModeloRoteiro_pkey" PRIMARY KEY ("id")
);

-- CreateTable
CREATE TABLE "OP" (
    "id" SERIAL NOT NULL,
    "numeroSequencia" INTEGER NOT NULL,
    "lote" TEXT,
    "modeloId" INTEGER NOT NULL,
    "quantidade" INTEGER NOT NULL,
    "status" TEXT NOT NULL DEFAULT 'ABERTA',
    "dataLiberacao" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "previsaoEntrega" TIMESTAMP(3),
    "createdAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "updatedAt" TIMESTAMP(3) NOT NULL,

    CONSTRAINT "OP_pkey" PRIMARY KEY ("id")
);

-- CreateTable
CREATE TABLE "Apontamento" (
    "id" SERIAL NOT NULL,
    "opId" INTEGER NOT NULL,
    "setorId" INTEGER NOT NULL,
    "usuario" TEXT NOT NULL,
    "quantidadeBoa" INTEGER NOT NULL DEFAULT 0,
    "quantidadeRefugo" INTEGER NOT NULL DEFAULT 0,
    "dataHora" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "soldador" TEXT,
    "bancada" TEXT,
    "abastecedor" TEXT,
    "quantidadeSoldada" INTEGER NOT NULL DEFAULT 0,
    "pecaId" INTEGER,
    "processo" TEXT,
    "roteiroEtapaId" INTEGER,
    "origem" TEXT NOT NULL DEFAULT 'OPERADOR',

    CONSTRAINT "Apontamento_pkey" PRIMARY KEY ("id")
);

-- CreateTable
CREATE TABLE "ConferenciaPCP" (
    "id" SERIAL NOT NULL,
    "setorId" INTEGER NOT NULL,
    "data" TIMESTAMP(3) NOT NULL,
    "usuario" TEXT NOT NULL,
    "totalConferido" INTEGER NOT NULL,
    "observacao" TEXT,
    "createdAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "updatedAt" TIMESTAMP(3) NOT NULL,

    CONSTRAINT "ConferenciaPCP_pkey" PRIMARY KEY ("id")
);

-- CreateTable
CREATE TABLE "ConferenciaPCPItem" (
    "id" SERIAL NOT NULL,
    "conferenciaId" INTEGER NOT NULL,
    "categoria" TEXT NOT NULL,
    "quantidade" INTEGER NOT NULL DEFAULT 0,

    CONSTRAINT "ConferenciaPCPItem_pkey" PRIMARY KEY ("id")
);

-- CreateIndex
CREATE UNIQUE INDEX "Setor_nome_key" ON "Setor"("nome");

-- CreateIndex
CREATE INDEX "Funcionario_setorId_idx" ON "Funcionario"("setorId");

-- CreateIndex
CREATE UNIQUE INDEX "Funcionario_nome_setorId_key" ON "Funcionario"("nome", "setorId");

-- CreateIndex
CREATE INDEX "AjusteApontamento_apontamentoId_idx" ON "AjusteApontamento"("apontamentoId");

-- CreateIndex
CREATE INDEX "AjusteApontamento_autorizadoPorId_idx" ON "AjusteApontamento"("autorizadoPorId");

-- CreateIndex
CREATE UNIQUE INDEX "Peca_codigo_key" ON "Peca"("codigo");

-- CreateIndex
CREATE INDEX "Peca_setorId_idx" ON "Peca"("setorId");

-- CreateIndex
CREATE INDEX "Peca_tipoMaterial_medidaA_medidaB_idx" ON "Peca"("tipoMaterial", "medidaA", "medidaB");

-- CreateIndex
CREATE UNIQUE INDEX "PecaCurva_pecaId_ordem_key" ON "PecaCurva"("pecaId", "ordem");

-- CreateIndex
CREATE INDEX "RecebimentoAgrupamento_setorOrigemId_idx" ON "RecebimentoAgrupamento"("setorOrigemId");

-- CreateIndex
CREATE INDEX "RecebimentoAgrupamento_dataHora_idx" ON "RecebimentoAgrupamento"("dataHora");

-- CreateIndex
CREATE UNIQUE INDEX "RecebimentoAgrupamento_opId_categoria_key" ON "RecebimentoAgrupamento"("opId", "categoria");

-- CreateIndex
CREATE INDEX "ModeloPeca_pecaId_idx" ON "ModeloPeca"("pecaId");

-- CreateIndex
CREATE UNIQUE INDEX "ModeloPeca_modeloId_pecaId_key" ON "ModeloPeca"("modeloId", "pecaId");

-- CreateIndex
CREATE INDEX "PecaRoteiro_setorId_idx" ON "PecaRoteiro"("setorId");

-- CreateIndex
CREATE INDEX "PecaRoteiro_pecaId_setorId_idx" ON "PecaRoteiro"("pecaId", "setorId");

-- CreateIndex
CREATE UNIQUE INDEX "PecaRoteiro_pecaId_ordem_key" ON "PecaRoteiro"("pecaId", "ordem");

-- CreateIndex
CREATE UNIQUE INDEX "Modelo_codigo_key" ON "Modelo"("codigo");

-- CreateIndex
CREATE INDEX "ModeloRoteiro_setorId_idx" ON "ModeloRoteiro"("setorId");

-- CreateIndex
CREATE UNIQUE INDEX "ModeloRoteiro_modeloId_setorId_key" ON "ModeloRoteiro"("modeloId", "setorId");

-- CreateIndex
CREATE UNIQUE INDEX "ModeloRoteiro_modeloId_ordem_key" ON "ModeloRoteiro"("modeloId", "ordem");

-- CreateIndex
CREATE UNIQUE INDEX "OP_lote_key" ON "OP"("lote");

-- CreateIndex
CREATE INDEX "OP_modeloId_idx" ON "OP"("modeloId");

-- CreateIndex
CREATE INDEX "OP_numeroSequencia_idx" ON "OP"("numeroSequencia");

-- CreateIndex
CREATE INDEX "Apontamento_opId_idx" ON "Apontamento"("opId");

-- CreateIndex
CREATE INDEX "Apontamento_setorId_idx" ON "Apontamento"("setorId");

-- CreateIndex
CREATE INDEX "Apontamento_pecaId_idx" ON "Apontamento"("pecaId");

-- CreateIndex
CREATE INDEX "Apontamento_roteiroEtapaId_idx" ON "Apontamento"("roteiroEtapaId");

-- CreateIndex
CREATE INDEX "Apontamento_setorId_dataHora_origem_idx" ON "Apontamento"("setorId", "dataHora", "origem");

-- CreateIndex
CREATE INDEX "ConferenciaPCP_data_idx" ON "ConferenciaPCP"("data");

-- CreateIndex
CREATE UNIQUE INDEX "ConferenciaPCP_setorId_data_key" ON "ConferenciaPCP"("setorId", "data");

-- CreateIndex
CREATE INDEX "ConferenciaPCPItem_conferenciaId_idx" ON "ConferenciaPCPItem"("conferenciaId");

-- CreateIndex
CREATE UNIQUE INDEX "ConferenciaPCPItem_conferenciaId_categoria_key" ON "ConferenciaPCPItem"("conferenciaId", "categoria");

-- AddForeignKey
ALTER TABLE "Funcionario" ADD CONSTRAINT "Funcionario_setorId_fkey" FOREIGN KEY ("setorId") REFERENCES "Setor"("id") ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "AjusteApontamento" ADD CONSTRAINT "AjusteApontamento_apontamentoId_fkey" FOREIGN KEY ("apontamentoId") REFERENCES "Apontamento"("id") ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "AjusteApontamento" ADD CONSTRAINT "AjusteApontamento_autorizadoPorId_fkey" FOREIGN KEY ("autorizadoPorId") REFERENCES "Funcionario"("id") ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "Peca" ADD CONSTRAINT "Peca_setorId_fkey" FOREIGN KEY ("setorId") REFERENCES "Setor"("id") ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "PecaCurva" ADD CONSTRAINT "PecaCurva_pecaId_fkey" FOREIGN KEY ("pecaId") REFERENCES "Peca"("id") ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "RecebimentoAgrupamento" ADD CONSTRAINT "RecebimentoAgrupamento_opId_fkey" FOREIGN KEY ("opId") REFERENCES "OP"("id") ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "RecebimentoAgrupamento" ADD CONSTRAINT "RecebimentoAgrupamento_setorOrigemId_fkey" FOREIGN KEY ("setorOrigemId") REFERENCES "Setor"("id") ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "ModeloPeca" ADD CONSTRAINT "ModeloPeca_modeloId_fkey" FOREIGN KEY ("modeloId") REFERENCES "Modelo"("id") ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "ModeloPeca" ADD CONSTRAINT "ModeloPeca_pecaId_fkey" FOREIGN KEY ("pecaId") REFERENCES "Peca"("id") ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "PecaRoteiro" ADD CONSTRAINT "PecaRoteiro_pecaId_fkey" FOREIGN KEY ("pecaId") REFERENCES "Peca"("id") ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "PecaRoteiro" ADD CONSTRAINT "PecaRoteiro_setorId_fkey" FOREIGN KEY ("setorId") REFERENCES "Setor"("id") ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "ModeloRoteiro" ADD CONSTRAINT "ModeloRoteiro_modeloId_fkey" FOREIGN KEY ("modeloId") REFERENCES "Modelo"("id") ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "ModeloRoteiro" ADD CONSTRAINT "ModeloRoteiro_setorId_fkey" FOREIGN KEY ("setorId") REFERENCES "Setor"("id") ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "OP" ADD CONSTRAINT "OP_modeloId_fkey" FOREIGN KEY ("modeloId") REFERENCES "Modelo"("id") ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "Apontamento" ADD CONSTRAINT "Apontamento_opId_fkey" FOREIGN KEY ("opId") REFERENCES "OP"("id") ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "Apontamento" ADD CONSTRAINT "Apontamento_setorId_fkey" FOREIGN KEY ("setorId") REFERENCES "Setor"("id") ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "Apontamento" ADD CONSTRAINT "Apontamento_pecaId_fkey" FOREIGN KEY ("pecaId") REFERENCES "Peca"("id") ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "Apontamento" ADD CONSTRAINT "Apontamento_roteiroEtapaId_fkey" FOREIGN KEY ("roteiroEtapaId") REFERENCES "PecaRoteiro"("id") ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "ConferenciaPCP" ADD CONSTRAINT "ConferenciaPCP_setorId_fkey" FOREIGN KEY ("setorId") REFERENCES "Setor"("id") ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "ConferenciaPCPItem" ADD CONSTRAINT "ConferenciaPCPItem_conferenciaId_fkey" FOREIGN KEY ("conferenciaId") REFERENCES "ConferenciaPCP"("id") ON DELETE CASCADE ON UPDATE CASCADE;
