...
UPDATE "PecaRoteiro"
SET "setorId" = (SELECT "id" FROM "Setor" WHERE UPPER(TRIM("nome")) = 'PLASMA TUBO' LIMIT 1), "ordem" = 1
WHERE "processo" = 'CORTE'
  AND "pecaId" IN (SELECT "id" FROM "Peca" WHERE "tipoMaterial" = 'PONTEIRA' AND (UPPER("nome") LIKE '%PONTEIRA REM%' OR UPPER("nome") LIKE '%CABECA REMOVIVEL%' OR UPPER("nome") LIKE '%CABEÇA REMOVÍVEL%'));

UPDATE "PecaRoteiro"
SET "setorId" = (SELECT "id" FROM "Setor" WHERE UPPER(TRIM("nome")) = 'PONTEIRA' LIMIT 1), "ordem" = 2
WHERE "processo" = 'SOLDAGEM'
  AND "pecaId" IN (SELECT "id" FROM "Peca" WHERE "tipoMaterial" = 'PONTEIRA' AND (UPPER("nome") LIKE '%PONTEIRA REM%' OR UPPER("nome") LIKE '%CABECA REMOVIVEL%' OR UPPER("nome") LIKE '%CABEÇA REMOVÍVEL%'));

UPDATE "PecaRoteiro"
SET "setorId" = (SELECT "id" FROM "Setor" WHERE UPPER(TRIM("nome")) = 'PONTEIRA' LIMIT 1), "ordem" = 3
WHERE "processo" = 'LIXAR'
  AND "pecaId" IN (SELECT "id" FROM "Peca" WHERE "tipoMaterial" = 'PONTEIRA' AND (UPPER("nome") LIKE '%PONTEIRA REM%' OR UPPER("nome") LIKE '%CABECA REMOVIVEL%' OR UPPER("nome") LIKE '%CABEÇA REMOVÍVEL%'));

UPDATE "PecaRoteiro"
SET "setorId" = (SELECT "id" FROM "Setor" WHERE UPPER(TRIM("nome")) = 'AGRUPAMENTO' LIMIT 1), "ordem" = 4
